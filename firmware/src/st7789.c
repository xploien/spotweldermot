...
static uint8_t _buffer[ST7789_WIDTH << 1] = {
    0}; // DMA buffer, long enough to fill a row.
...
static void SPI_init(void) {
  ...
  SPI1->CTLR1 =
      ...
      | SPI_CPOL_High // Bit 1     - Clock POLarity - idles at the logical high
                      // voltage
...
static void SPI_send_DMA(const uint8_t *buffer, uint16_t size,
                         uint16_t repeat) {
...
void tft_init(void) {
  ...
  // write_data_8(ST7789_MADCTL_MY | ST7789_MADCTL_MV | ST7789_MADCTL_RGB);  //
  // 0 - Horizontal - X-Y Swap & Y-Mirror
  write_data_8(ST7789_MADCTL_RGB); // 1 - Vertical
  // write_data_8(ST7789_MADCTL_MX | ST7789_MADCTL_MV | ST7789_MADCTL_RGB);  //
  // 2 - Horizontal - X-Y Swap & X-Mirror write_data_8(ST7789_MADCTL_MX |
  // ST7789_MADCTL_MY | ST7789_MADCTL_RGB);  // 3 - Vertical   - X-Mirror &
  // Y-Mirror

  // write_command_8(ST7789_RAMCTRL);
  // write_data_8(0x00);  // Power on default - RAM access from MCU interface
  // and Display operation selection from MCU
  //                      // interface.
  // write_data_8(0xF0);  // Power on default - Data translate EPF = 0x11 and
  // MSB first.
  ...
  // write_data_8(0b11110000);  // V63P3, V63P2, V63P1, V63P0,  V0P3,  V0P2,
  // V0P1,  V0P0 write_data_8(0b00001001);  //     0,     0,  V1P5,  V1P4, V1P3,
  // V1P2,  V1P1,  V1P0 write_data_8(0b00010011);  //     0,     0,  V2P5, V2P4,
  // V2P3,  V2P2,  V2P1,  V2P0 write_data_8(0b00010010);  //     0,     0, 0,
  // V4P4,  V4P3,  V4P2,  V4P1,  V4P0 write_data_8(0b00010010);  //     0, 0, 0,
  // V6P4,  V6P3,  V6P2,  V6P1,  V6P0 write_data_8(0b00101011);  //     0, 0,
  // J0P1,  J0P0, V13P3, V13P2, V13P1, V13P0 write_data_8(0b00111100);  // 0,
  // V20P6, V20P5, V20P4, V20P3, V20P2, V20P1, V20P0 write_data_8(0b01000100);
  // //     0, V36P2, V36P1, V36P0,     0, V27P2, V27P1, V27P0
  // write_data_8(0b01001011);  //     0, V43P6, V43P5, V43P4, V43P3, V43P2,
  // V43P1, V43P0 write_data_8(0b00011011);  //     0,     0,  J1P1,  J1P0,
  // V50P3, V50P2, V50P1, V50P0 write_data_8(0b00011000);  //     0,     0, 0,
  // V57P4, V57P3, V57P2, V57P1, V57P0 write_data_8(0b00010111);  //     0, 0,
  // 0, V59P4, V59P3, V59P2, V59P1, V59P0 write_data_8(0b00011101);  //     0,
  // 0, V61P5, V61P4, V61P3, V61P2, V61P1, V61P0 write_data_8(0b00100001);  //
  // 0,     0, V62P5, V62P4, V62P3, V62P2, V62P1, V62P0

  // write_command_8(ST7789_NVGAMCTRL);
  // write_data_8(0b11110000);  // V63P3, V63P2, V63P1, V63P0,  V0P3,  V0P2,
  // V0P1,  V0P0 write_data_8(0b00001001);  //     0,     0,  V1P5,  V1P4, V1P3,
  // V1P2,  V1P1,  V1P0 write_data_8(0b00010011);  //     0,     0,  V2P5, V2P4,
  // V2P3,  V2P2,  V2P1,  V2P0 write_data_8(0b00001100);  //     0,     0, 0,
  // V4N4,  V4N3,  V4N2,  V4N1,  V4N0 write_data_8(0b00001101);  //     0, 0, 0,
  // V6N4,  V6N3,  V6N2,  V6N1,  V6N0 write_data_8(0b00100111);  //     0, 0,
  // J0N1,  J0N0, V13N3, V13N2, V13N1, V13N0 write_data_8(0b00111011);  // 0,
  // V20N6, V20N5, V20N4, V20N3, V20N2, V20N1, V20N0 write_data_8(0b01000100);
  // //     0, V36N2, V36N1, V36N0,     0, V27N2, V27N1, V27N0
  // write_data_8(0b01001101);  //     0, V43N6, V43N5, V43N4, V43N3, V43N2,
  // V43N1, V43N0 write_data_8(0b00001011);  //     0,     0,  J1N1,  J1N0,
  // V50N3, V50N2, V50N1, V50N0 write_data_8(0b00010111);  //     0,     0, 0,
  // V57N4, V57N3, V57N2, V57N1, V57N0 write_data_8(0b00010111);  //     0, 0,
  // 0, V59N4, V59N3, V59N2, V59N1, V59N0 write_data_8(0b00011101);  //     0,
  // 0, V61N5, V61N4, V61N3, V61N2, V61N1, V61N0 write_data_8(0b00100001);  //
  // 0,     0, V62N5, V62N4, V62N3, V62N2, V62N1, V62N0
...
void tft_set_color(uint16_t color) { _color = color; }

/// \brief Set Text Background Color
/// \param color Text background color
/// \details Set to `_bg_color` variable
void tft_set_background_color(uint16_t color) { _bg_color = color; }
...
void tft_print_char(char c) {
  ...
  START_WRITE();
  tft_set_window(_cursor_x, _cursor_y, _cursor_x + FONT_WIDTH - 1,
...
/// \details DMA accelerated
static void _tft_draw_fast_v_line(int16_t x, int16_t y, int16_t h,
                                  uint16_t color) {
  x += ST7789_X_OFFSET;
  y += ST7789_Y_OFFSET;
...
/// \param color Line color
/// \details DMA accelerated
static void _tft_draw_fast_h_line(int16_t x, int16_t y, int16_t w,
                                  uint16_t color) {
  x += ST7789_X_OFFSET;
  y += ST7789_Y_OFFSET;
...
/// \param color Line color
/// \details SPI direct write
static void _tft_draw_line_bresenham(int16_t x0, int16_t y0, int16_t x1,
...
/// \param y1 End Y coordinate
/// \param color Line color
void tft_draw_rect(uint16_t x, uint16_t y, uint16_t width, uint16_t height,
                   uint16_t color) {
...
void tft_draw_thick_circle(int16_t cx, int16_t cy, int16_t outer_r,
                           int16_t inner_r, uint16_t color) {
  /* Recorre cada fila del bounding-box */
  for (int16_t dy = -outer_r; dy <= outer_r; dy++) {

    /* Semiancho exterior en esta fila */
    int32_t dy2 = (int32_t)dy * dy;
    int32_t or2 = (int32_t)outer_r * outer_r;
    int32_t dx_outer2 = or2 - dy2;
    if (dx_outer2 < 0)
      continue; /* fuera del círculo */

    /* isqrt entero — sin floats, seguro en MCU */
    int16_t ox = (int16_t)isqrt32(dx_outer2);

    /* Semiancho interior en esta fila */
    int32_t ir2 = (int32_t)inner_r * inner_r;
    int32_t dx_inner2 = ir2 - dy2;
    int16_t ix = (dx_inner2 > 0) ? (int16_t)isqrt32(dx_inner2) : 0;

    int16_t y = cy + dy;

    if (ix == 0) {
      /* Fila pasa por el "polo": un único tramo lleno */
      tft_fill_rect(cx - ox, y, 2 * ox + 1, 1, color);
    } else {
      /* Tramo izquierdo y tramo derecho del anillo */
      tft_fill_rect(cx - ox, y, ox - ix, 1, color);
      tft_fill_rect(cx + ix, y, ox - ix + 1, 1, color);
    }
  }
}
